...
public class SysUserServiceImpl implements SysUserService {
    ...
    public UserVo login(SysUser sysUser) {
        ...
        SimpleHash hash = new SimpleHash("md5", sysUser.getPassword(), user.getSalt(), user.getHashIterations());
        if (!hash.toString().equals(user.getPassword())) {
            throw new CustomException(RCode.INCORRECT_CREDENTIAL);
        }
        ...
        RolesAndMenus rolesAndMenus = getRealRolesAndMenus(user.getId());
